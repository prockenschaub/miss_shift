...
fields = ['iter', 'method', 'n', 'mse_train', 'mse_val', 'mse_test', 'mse_test_m', 'mse_test_s',
          ...
          'link', 'data_path', 'curvature', 'width_factor', 'max_leaf_nodes', 'min_samples_leaf',
          'use_y_for_impute',
...
def launch(args):
    ...
    data_spec = experiment['data'][args.data]
    ...
    for nm, scope in methods_params.items():
        ...
        print(f'Run {len(scope)} hyperparam combinations for {args.n_trials} trials, resulting in {len(runs)} runs')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tmp_dir', help='directory where to store any other files (e.g., pre-trained imputation models)', type=str, default='tmp')
